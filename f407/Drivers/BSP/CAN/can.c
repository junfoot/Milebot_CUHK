...
extern float pos_origin[4];
extern float cur_origin[4];
extern short cur_o;
...
//    __HAL_CAN_ENABLE_IT(&g_canx_handler, CAN_IT_RX_FIFO0_MSG_PENDING); /* FIFO0消息挂号中断允许 */
	...
	HAL_CAN_ActivateNotification(&g_canx_handler,CAN_IT_RX_FIFO0_MSG_PENDING);//使能中断
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    {
        rxlen = can_receive_msg(0x0555, canbuf);
        if(rxlen)
        {
            if(canbuf[1]==0x05)
            {
                // pos值转换为角度
                pos_origin[canbuf[0]-1] = (float)((canbuf[2]<<24)+(canbuf[3]<<16)+(canbuf[4]<<8)+canbuf[5]) * 5.52709e-4f;
            }
            
    //			else if(canbuf[1]==0x3F)
    //			{
    //				pos_vel_cur[canbuf[0]+3] = (double)((canbuf[2]<<24)+(canbuf[3]<<16)+(canbuf[4]<<8)+canbuf[5])*0.1f;
    //			}
            
            else if(canbuf[1]==0x40)
            {
                cur_o = (canbuf[4]<<8)+canbuf[5];
                cur_origin[canbuf[0]-1] = (float)cur_o*0.001f;
            }
        }
    }
}
