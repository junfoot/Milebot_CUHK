...
float LeftTorquePrev = 0, RightTorquePrev = 0;
...
{   
    ...
    while(1) 
    {
        ...
        else if ( *p == 'x' && *(p+1) == ':')   // cmd from PC
        {
            ...
            left_pos = exo_state[0];
            right_pos = exo_state[3];
...
void task4(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        // for tuning
        len += snprintf(buf + len, sizeof(buf) - len, "%7.3f,%7.3f,", LeftTorque, RightTorque);
