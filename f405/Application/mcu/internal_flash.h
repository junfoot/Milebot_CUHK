...
typedef enum
{
    ...
    FLASH_PARAM_SN_BEGIN,      //设备SN 号
    FLASH_PARAM_SN_END = (FLASH_PARAM_SN_BEGIN + DEVICE_SN_LEN - 1),
    FLASH_PARAM_SN_RESERVE_1,  //保留, 为了能够4 字节对齐
    FLASH_PARAM_SN_RESERVE_2,  //保留, 为了能够4 字节对齐
    
    PARAM_BUF_LEN,
}FLASH_PARAM_INDEX; 

extern uint8_t param_buf[PARAM_BUF_LEN];
extern uint8_t device_SN_buf[DEVICE_SN_LEN + 1];
