...
#endif	/* CO_NO_CAN_MODULES */
    static CO_OD_extension_t   *CO_SDO_ODExtensions;
    static CO_HBconsNode_t     *CO_HBcons_monitoredNodes0;
#if CO_NO_CAN_MODULES >= 2	
	static CO_HBconsNode_t     *CO_HBcons_monitoredNodes1;
#endif
#if CO_NO_TRACE > 0
...
/* Indexes for CANopenNode message objects ************************************/
    ...
    #define CO_RXCAN_NO_MSGS (1+CO_NO_SYNC+CO_NO_RPDO/2+CO_NO_SDO_SERVER+CO_NO_SDO_CLIENT+CO_NO_HB_CONS/2)

	#if CO_NO_CAN_MODULES >= 2
	#define CO_RXCAN2_CONS_HB	(CO_NO_RPDO/2)
	/* NMT RPDO0~3  */
	#define CO_RXCAN2_NO_MSGS	(CO_NO_RPDO/2+CO_NO_HB_CONS/2) /* Modify macro define based actual application */
	#endif
	...
	#if CO_NO_CAN_MODULES >= 2
	/* Modify macro define based actual application */
	#define CO_TXCAN2_NO_MSGS	(CO_NO_NMT_MASTER+CO_NO_TPDO+CO_NO_EMERGENCY)
	#endif
  ...
  #if CO_NO_CAN_MODULES >= 2
    static CO_CANrx_t           COO_CANmodule_rxArray1[CO_RXCAN2_NO_MSGS];
    static CO_CANtx_t           COO_CANmodule_txArray1[CO_TXCAN2_NO_MSGS];
  #endif
    ...
    static CO_HBconsumer_t      COO_HBcons[CO_NO_CAN_MODULES];
...
#if CO_NO_NMT_MASTER == 1
	#if CO_NO_CAN_MODULES >= 2
    CO_CANtx_t *NMTM2_txBuff = 0;
	int8_t CO_sendNMT2command(CO_t *CO, uint8_t command, uint8_t nodeID)
	{
        if(NMTM2_txBuff == 0)
		{
        ...
        NMTM2_txBuff->data[0] = command;
        NMTM2_txBuff->data[1] = nodeID;
        ...
        return CO_CANsend(CO->CANmodule[1], NMTM2_txBuff); /* 0 = success */
    }
	#endif
...
void CO_CAN_TxCpltCallback(CAN_TypeDef* CANx)
{
	if(CANx == CAN1)
	{
		CO_CANinterrupt_Tx(COO.CANmodule[0]);
	}
	else if(CANx == CAN2)
	{
		CO_CANinterrupt_Tx(COO.CANmodule[1]);
	}
}

void CO_CAN_RxCpltCallback(CAN_TypeDef* CANx, CanRxMsgTypeDef* pRxMsg)
{
	if(CANx == CAN1)
	{
		CO_CANinterrupt_Rx(pRxMsg,COO.CANmodule[0]);
	}
	else if(CANx == CAN2)
	{
		CO_CANinterrupt_Rx(pRxMsg,COO.CANmodule[1]);
	}
        ...
        uint16_t                bitRate)
{
    ...
    CO->HBcons[0]                       = &COO_HBcons[0];
	#if CO_NO_CAN_MODULES >= 2
	CO->HBcons[1]                       = &COO_HBcons[1];
	#endif
    CO_HBcons_monitoredNodes0            = &COO_HBcons_monitoredNodes[0];
	#if CO_NO_CAN_MODULES >= 2
	CO_HBcons_monitoredNodes1            = &COO_HBcons_monitoredNodes[CO_NO_HB_CONS/2];
	#endif
  #if CO_NO_SDO_CLIENT == 1
    ...
    if(CO == NULL){    /* Use malloc only once */
	  ...
	  #if CO_NO_CAN_MODULES >= 2
    	CO->CANmodule[1]                    = (CO_CANmodule_t *)    calloc(1, sizeof(CO_CANmodule_t));
        CO_CANmodule_rxArray1               = (CO_CANrx_t *)        calloc(CO_RXCAN2_NO_MSGS, sizeof(CO_CANrx_t));
        CO_CANmodule_txArray1               = (CO_CANtx_t *)        calloc(CO_TXCAN2_NO_MSGS, sizeof(CO_CANtx_t));
  	  #endif
        for(i=0; i<CO_NO_SDO_SERVER; i++){
        ...
        CO->HBcons[0]                       = (CO_HBconsumer_t *)   calloc(1, sizeof(CO_HBconsumer_t));
		CO_HBcons_monitoredNodes0           = (CO_HBconsNode_t *)   calloc(CO_NO_HB_CONS/2, sizeof(CO_HBconsNode_t));
      #if CO_NO_CAN_MODULES >= 2
		CO->HBcons[1]                       = (CO_HBconsumer_t *)   calloc(1, sizeof(CO_HBconsumer_t));
	    CO_HBcons_monitoredNodes1           = (CO_HBconsNode_t *)   calloc(CO_NO_HB_CONS/2, sizeof(CO_HBconsNode_t));
	  #endif
    ...
    CO_memoryUsed = sizeof(CO_CANmodule_t)
                  ...
                  + sizeof(CO_CANrx_t) * CO_RXCAN2_NO_MSGS
                  + sizeof(CO_CANtx_t) * CO_TXCAN2_NO_MSGS
    #endif             
	...
	if(CO->HBcons[1]                    == NULL) errCnt++;
    if(CO_HBcons_monitoredNodes1        == NULL) errCnt++;
  #endif
    for(i=0; i<CO_NO_SDO_SERVER; i++){
    ...
    if(CO->HBcons[0]                    == NULL) errCnt++;
    if(CO_HBcons_monitoredNodes0        == NULL) errCnt++;
  #if CO_NO_SDO_CLIENT == 1
    ...
    CO_CANsetConfigurationMode(CANx);
	#if CO_NO_CAN_MODULES >= 2
	CO->CANmodule[1]->CANnormal = false;
    CO_CANsetConfigurationMode(CAN2);
	#endif
    ...
    err = CO_CANmodule_init(
            ...
            CO_TXCAN_NO_MSGS,
            bitRate);
    if(err){CO_delete(CANx); return err;}
    ...
    err = CO_CANmodule_init(
            ...
            CO_TXCAN2_NO_MSGS,
            bitRate);
    if(err){CO_delete(CAN2); return err;}
	#endif
   ...
   #if CO_NO_CAN_MODULES >= 2
   NMTM2_txBuff = CO_CANtxBufferInit(/* return pointer to 8-byte CAN data buffer, which should be populated */
            CO->CANmodule[1], /* pointer to CAN module used for sending this message */
            CO_TXCAN_NMT,     /* index of specific buffer inside CAN module */
            0x0000,           /* CAN identifier */
            0,                /* rtr */
            2,                /* number of data bytes */
	...
	err = CO_HBconsumer_init(
            ...
            CO->em,
            CO->SDO[0],
           &OD_consumerHeartbeatTime[CO_NO_HB_CONS/2],
            CO_HBcons_monitoredNodes1,
            CO_NO_HB_CONS/2,
            CO->CANmodule[1],
            CO_RXCAN2_CONS_HB);
    if(err){CO_delete(CANx); return err;}
...
/******************************************************************************/
void CO_delete(CAN_TypeDef* CANx){
#ifndef CO_USE_GLOBALS
    int16_t i;
#endif
    ...
    free(CO->SDOclient);
  #endif
    free(CO_HBcons_monitoredNodes0);
    free(CO->HBcons[0]);
    for(i=0; i<CO_NO_RPDO; i++){
        free(CO->RPDO[i]);
    }
    ...
    free(CO->CANmodule[0]);
	#if CO_NO_CAN_MODULES >= 2
	free(CO_HBcons_monitoredNodes1);
	free(CO->HBcons[1]);
