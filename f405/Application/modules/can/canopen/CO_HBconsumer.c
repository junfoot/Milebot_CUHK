...
static void CO_HBcons_receive(void *object, const CO_CANrxMsg_t *msg){
    ...
    if(msg->DLC == 1){
		...
		HBconsNode->ident = msg->ident;
		if(HBconsNode->NMTstate == CO_NMT_PRE_OPERATIONAL)
		{
			HBconsNode->pre_opste_cnt++;
			if(HBconsNode->pre_opste_cnt > 5)
			{
				HBconsNode->pre_opste_cnt = 0;
                #if CO_NO_NMT_MASTER == 1
				if(HBconsNode->ident == CIA_NODE_C1_ID)
				{
					CO_sendNMTcommand(&COO, CO_NMT_RESET_COMMUNICATION, HBconsNode->ident);
				}
				else if(HBconsNode->ident == CIA_NODE_C2_ID)
				{
                    #if CO_NO_CAN_MODULES>=2
					CO_sendNMT2command(&COO, CO_NMT_RESET_COMMUNICATION, HBconsNode->ident);
                    #endif
				}
                #endif
			}
			else
			{
				...
				if(HBconsNode->ident == CIA_NODE_C1_ID)
				{
					CO_sendNMTcommand(&COO, CO_NMT_ENTER_OPERATIONAL, HBconsNode->ident);
				}
                
				else if(HBconsNode->ident == CIA_NODE_C2_ID)
				{
                 #if CO_NO_CAN_MODULES>=2
					CO_sendNMT2command(&COO, CO_NMT_ENTER_OPERATIONAL, HBconsNode->ident);
				 #endif
                    }
                #endif
        ...
        uint32_t                HBconsTime)
{
	...
	monitoredNode->ident = 0;
	monitoredNode->pre_opste_cnt = 0;
        ...
        uint16_t                CANdevRxIdxStart)
{
    ...
    /* Configure Object dictionary entry at 0x1016 */
