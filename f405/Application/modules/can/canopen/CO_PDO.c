...
static void CO_PDO_receive(void *object, const CO_CANrxMsg_t *msg){
    ...
    RPDO = (CO_RPDO_t*)object;   /* this is the correct pointer type of the first argument */
		...
		memset(RPDO->CANrxData[0],0x00,8);
		memset(RPDO->CANrxData[1],0x00,8);
    if( (RPDO->valid) &&
        (*RPDO->operatingState == CO_NMT_OPERATIONAL)&&msg->DLC!=0)
        //&&(msg->DLC >= RPDO->dataLength))
    {
        if(RPDO->synchronous && RPDO->SYNC->CANrxToggle) {
            ...
            //if(RPDO == COO.RPDO[0])
                //can_app_rpdo_process(0,(uint16_t*)&OD_status_word);
...
#if 0 
uint8_t CO_RPDOisCOS(CO_RPDO_t *RPDO){
    ...
    ppODdataByte = &RPDO->mapPointer[RPDO->dataLength];

    switch(RPDO->dataLength){
        case 8: if(*(--pPDOdataByte) != **(--ppODdataByte) && (RPDO->sendIfCOSFlags&0x80)) return 1;
        case 7: if(*(--pPDOdataByte) != **(--ppODdataByte) && (RPDO->sendIfCOSFlags&0x40)) return 1;
        case 6: if(*(--pPDOdataByte) != **(--ppODdataByte) && (RPDO->sendIfCOSFlags&0x20)) return 1;
        case 5: if(*(--pPDOdataByte) != **(--ppODdataByte) && (RPDO->sendIfCOSFlags&0x10)) return 1;
        case 4: if(*(--pPDOdataByte) != **(--ppODdataByte) && (RPDO->sendIfCOSFlags&0x08)) return 1;
        case 3: if(*(--pPDOdataByte) != **(--ppODdataByte) && (RPDO->sendIfCOSFlags&0x04)) return 1;
        case 2: if(*(--pPDOdataByte) != **(--ppODdataByte) && (RPDO->sendIfCOSFlags&0x02)) return 1;
        case 1: if(*(--pPDOdataByte) != **(--ppODdataByte) && (RPDO->sendIfCOSFlags&0x01)) return 1;
    }
