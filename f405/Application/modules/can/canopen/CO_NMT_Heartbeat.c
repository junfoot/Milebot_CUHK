 ...
 */
 /*这个函数是收到NMT master发下来的指令进行计算，这里主机为NMT master ,因此此函数无用*/
static void CO_NMT_receive(void *object, const CO_CANrxMsg_t *msg){
    ...
    if((msg->DLC == 2) && ((nodeId == 0) || (nodeId == NMT->nodeId))){
        ...
        if(NMT->pFunctNMT!=NULL && currentOperatingState!=NMT->operatingState){
            NMT->pFunctNMT(object, (CO_NMT_internalState_t)NMT->operatingState);
        }
    }
}

static void CO_NMT_FunctCallback(void *object, CO_NMT_internalState_t state)
{
        ...
        uint16_t                CANidTxHB)
{
    ...
    NMT->emPr                   = emPr;
    NMT->pFunctNMT              = CO_NMT_FunctCallback;
...
void CO_NMT_initCallback(
        CO_NMT_t               *NMT,
        void                  (*pFunctNMT)(void *object, CO_NMT_internalState_t state))
{
    if(NMT != NULL){
        NMT->pFunctNMT = pFunctNMT;
        if(NMT->pFunctNMT != NULL){
            NMT->pFunctNMT(NMT, (CO_NMT_internalState_t)NMT->operatingState);
        }
    }
}
